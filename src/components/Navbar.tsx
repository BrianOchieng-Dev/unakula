...
export function Navbar({ user, onLogin, onLogout, onProfileClick, onMyPostsClick, onAddClick, isDarkMode, toggleDarkMode }: NavbarProps) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-3">
      <div className="max-w-2xl mx-auto flex items-center justify-between bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl px-6 py-2 shadow-lg">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="rounded-xl text-blue-200 hover:bg-white/5 h-9 w-9"
          >
          ...
          {user ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 bg-slate-900/95 backdrop-blur-xl border-white/10 text-white rounded-2xl">
                <DropdownMenuGroup>
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-bold leading-none">{user.displayName}</p>
                      <p className="text-[10px] leading-none text-blue-200/50">{user.email}</p>
                    </div>
                  </DropdownMenuLabel>
                </DropdownMenuGroup>
                <DropdownMenuSeparator className="bg-white/5" />
                <DropdownMenuItem onClick={onProfileClick} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 rounded-lg m-1">
                  <UserIcon className="w-4 h-4" />
                  <span className="text-xs">My Profile</span>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={onMyPostsClick} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 rounded-lg m-1">
