      ...
      <div className="space-y-8">
            ...
            <div className="p-[3px] rounded-full bg-gradient-to-tr from-blue-600 to-blue-400">
              <div className="p-[2px] bg-slate-950 rounded-full">
                <Avatar className="h-24 w-24 md:h-32 md:w-32 border-none">
                  ...
                  <AvatarFallback className="bg-blue-600 text-4xl font-bold">
          ...
          <div className="flex-1 space-y-4 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  className="bg-white/10 hover:bg-white/20 text-white border-none rounded-lg h-8 px-4 text-xs font-bold"
                >
            ...
            <div className="flex items-center justify-center md:justify-start gap-8">
              <div className="text-center md:text-left">
                ...
                <span className="text-xs text-blue-200/50">posts</span>
              </div>
              <div className="text-center md:text-left">
                <span className="block text-lg font-bold text-white">{profile.followersCount || 0}</span>
                <span className="text-xs text-blue-200/50">followers</span>
              </div>
              <div className="text-center md:text-left">
                <span className="block text-lg font-bold text-white">{profile.followingCount || 0}</span>
                <span className="text-xs text-blue-200/50">following</span>
              </div>
            </div>

            <div className="space-y-1">
              <p className="text-sm font-bold text-white">{formData.studentId}</p>
              <p className="text-xs text-blue-200/70">MMUST Student • {formData.yearOfStudy} Year</p>
            </div>
          </div>
        </div>
          ...
          {showDeleteConfirm && (
            ...
            >
              <motion.div
                ...
                className="bg-slate-900 border border-white/10 p-6 rounded-2xl max-w-sm w-full shadow-2xl space-y-6"
              >
        ...
        <div className="px-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Active Streaks</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <Avatar className="h-10 w-10 border border-white/10">
                      <AvatarFallback className="bg-blue-600 text-xs font-bold">SP</AvatarFallback>
                    </Avatar>
                  ...
                  <div>
                    <p className="text-xs font-bold text-white">Partner Streak</p>
                    <p className="text-[10px] text-blue-200/50">Keep posting to grow!</p>
                  </div>
                </div>
            ...
            {streaks.length === 0 && (
              <p className="text-xs text-blue-200/30 italic">No active streaks. Follow friends to start one!</p>
            )}
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="bg-white/5 border-white/10 text-white focus:border-blue-500 h-10"
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs text-blue-200/50">Year of Study</Label>
                    <Input
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <Label className="text-xs text-blue-200/50">Student ID</Label>
                    <Input
                      value={formData.studentId}
                      onChange={(e) => setFormData({ ...formData, studentId: e.target.value.toUpperCase() })}
                      className={`bg-white/5 border-white/10 text-white focus:border-blue-500 h-10 ${!isIdValid ? 'border-red-500/50' : ''}`}
                      required
                    />
                  </div>
                </div>
        ...
        <div className="border-t border-white/10 pt-8">
          <div className="flex items-center justify-center gap-12 mb-8">
            <div className="flex items-center gap-2 text-white border-t-2 border-white pt-2 -mt-[34px]">
          ...
          {userPosts.length === 0 && (
            <div className="text-center py-20">
              <Camera className="w-12 h-12 text-blue-200/20 mx-auto mb-4" />
              <h3 className="text-lg font-bold text-white">No Posts Yet</h3>
              <p className="text-sm text-blue-200/50">When you share meals, they'll appear here.</p>
            </div>
          )}
        </div>
      </div>
